function Get-ExposureByHash {
    ...
    Begin {
        ...
        if ($PSBoundParameters.ContainsKey('ApiKey')) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($apiKeyValue)) {
                $trimmedApiKey = $apiKeyValue.Trim()
                if ($trimmedApiKey) {
                    Test-ValidHibpApiKey -ApiKey $trimmedApiKey -ThrowOnFail
                    $headers.'hibp-api-key' = $trimmedApiKey
                }
            }
        }

        $invokeDefaults = @{}
        $scriptIwrParams = Get-Variable -Name IWR_PARAMS -Scope Script -ErrorAction SilentlyContinue
        if ($scriptIwrParams -and $scriptIwrParams.Value) {
            foreach ($key in $scriptIwrParams.Value.Keys) {
                $invokeDefaults[$key] = $scriptIwrParams.Value[$key]
            }
        }

        if ($PSBoundParameters.ContainsKey('UserAgent')) {
            $userAgentValue = $UserAgent
            if (-not [string]::IsNullOrWhiteSpace($userAgentValue)) {
                $invokeDefaults['UserAgent'] = $userAgentValue
            }
        }
    }
    ...
    Process
    {
        ...
        if ($ApiRoot -like 'http*') {
            ...
            try {
                $results = (Invoke-WebRequest @queryParams @invokeDefaults).Content
    ...
    .PARAMETER ApiKey
        If specified, adds the hibp-api-key header to authenticated Have I Been Pwned requests.

    .PARAMETER UserAgent
        If specified, overrides the default user agent when performing Have I Been Pwned requests.
    ...
    .EXAMPLE
        ...
        PS C:\>$hash | Get-ExposureByHash -ApiRoot 'http://internal.example.com/range/'
